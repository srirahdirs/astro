...
type LookupResult = {
  ...
  profileDetailsReceivedFrom: { sender_registration_id: string; fields_sent: string; sent_at: string; sender_name?: string; sender_role?: string }[];
};

type RegistrationOption = { registration_id: string; name: string; role: string };

export default function ReceivedProfileSearchPage() {
  ...
  return (
    <div className="container-dashboard">
      ...
      <h1 className="page-title">Details received from</h1>
      <p className="text-navy-600 mb-6">
        Search by profile ID to see who sent their details to this profile.
      </p>
      ...
      {result && (
        <div className="space-y-6">
          ...
          <div className="card min-w-0">
            <h2 className="section-title-accent mb-3">Profile details received from</h2>
            <p className="text-xs text-navy-500 mb-2">Profiles who sent their details to this profile (via Send profile details).</p>
            {result.profileDetailsReceivedFrom.length === 0 ? (
              <p className="text-navy-600 text-sm py-2">No profile details received yet.</p>
            ) : (
                ...
                <table className="w-full min-w-[280px] text-sm">
                  <thead>
                    <tr className="border-b border-navy-200 bg-navy-50/50">
                      <th className="text-left p-2.5 font-semibold text-navy-700">From profile</th>
                  ...
                  <tbody>
                    {result.profileDetailsReceivedFrom.map((r, i) => {
                      const fields = typeof r.fields_sent === 'string' ? (() => { try { return JSON.parse(r.fields_sent); } catch { return []; } })() : (r.fields_sent || []);
                      return (
                        <tr key={`${r.sender_registration_id}-${r.sent_at}-${i}`} className="border-b border-navy-100 last:border-0 hover:bg-navy-50/30">
                          <td className="p-2.5 align-top">
                            <Link href={`/dashboard/received-profile-search?id=${r.sender_registration_id}`} className="link font-medium">
                              {r.sender_registration_id}
                            </Link>
                            {r.sender_name && <span className="text-navy-600 ml-1">({r.sender_name})</span>}
                          </td>
