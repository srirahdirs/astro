...
export default function HoroscopeProfileSearchPage() {
  ...
  return (
    <div className="container-dashboard">
      ...
      <h1 className="page-title">Horoscope sent to</h1>
      <p className="text-navy-600 mb-6">
        Search by profile ID to see who received their horoscope PDF (from Upload horoscope).
      </p>
                      ...
                      <tr key={`${r.recipient_whatsapp}-${r.sent_at}-${i}`} className="border-b border-navy-100 last:border-0 hover:bg-navy-50/30">
                        <td className="p-2.5 align-top">
                          {r.match_registration_id ? (
                            <Link href={`/dashboard/horoscope-profile-search?id=${r.match_registration_id}`} className="link font-medium">
                              {r.match_registration_id}
                            </Link>
                          ) : (
